...
export default function MarketExchangeForm() {
    ...
    const customerFaultReasons = ["규격/사이즈가 현장 조건과 맞지 않음", "색상 또는 옵션을 잘못 선택함", "필요한 사이즈/길이/두께를 잘못 선택함"];
    ...
    const getOptions = (productIdx) => {
        myAxios(token, setToken)
            .get("http://localhost:8080" + `/market/product/options?productIdx=${productIdx}`)
            .then((res) => {
                setOptions((prev) => ({
                    ...prev,
                    [productIdx]: res.data,
    ...
    const submitExchange = () => {
        ...
        const exchangeItems = Object.entries(selectedOptions).map(([orderItemIdx, newOptionIdx]) => ({
        ...
        formData.append("exchangeRequest", new Blob([JSON.stringify(exchangeRequest)], { type: "application/json" }));
    ...
    return (
        <div className="mypage-layout">
                                        ...
                                        <td style={{ padding: "0 10px" }}>
                                            ...
                                            >
                                                ...
                                                {options[item.productIdx]?.map((opt) => (
                                                    <option key={opt.productOptionIdx} value={opt.productOptionIdx}>
                                                        {opt.name} - {opt.value}
                                                        {opt.price > item.price && ` (+${(item.price - opt.price).toLocaleString()}원)`}
                                                    </option>
                                                ))}
                                            </Input>
                                        </td>
            ...
            <div>
                ...
                <div className="labelInput-wrapper" style={{ borderBottom: "none", padding: "10px 0" }}>
                ...
                <div className="labelInput-wrapper" style={{ padding: "10px 0 16px 0" }}>
                ...
                <div className="labelInput-wrapper">
                    ...
                    >
                        <div className="mypage-radio">
                            <input id="shippingPayer" type="radio" name="shippingPayer" checked={shippingChargeType === "SELLER"} disabled />
                            <laebl for="shippingPayer">판매자</laebl>
                        </div>
                        <div className="mypage-radio">
                            <input id="shippingPayer" type="radio" name="shippingPayer" checked={shippingChargeType === "BUYER"} disabled />
